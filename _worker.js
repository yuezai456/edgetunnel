...
function 返回SS接口JSON(body, status = 200) {
...
function 生成SS链接(node) {
	...
	return `ss://${encoded}@${node.ip}:${node.port}`;
}

function 生成SOCKS5链接(node) {
	const auth = node.username && node.password ? `${encodeURIComponent(node.username)}:${encodeURIComponent(node.password)}@` : '';
	return `socks5://${auth}${node.ip}:${node.port}`;
}

function 校验并整理SOCKS5节点(nodes) {
	if (!Array.isArray(nodes)) return [];
	return nodes.filter(node => node && node.enabled === true && typeof node.ip === 'string' && node.ip.trim() && Number.isInteger(node.port) && node.port > 0 && node.port <= 65535)
		.map(node => ({ ip: node.ip.trim(), port: node.port, username: typeof node.username === 'string' ? node.username : '', password: typeof node.password === 'string' ? node.password : '' }));
}
...
export default {
	async fetch(request, env, ctx) {
		...
		} else if (访问路径 === 'api/socks5' && request.method === 'GET') {// SOCKS5节点接口
			const SS_API_TOKEN = env.SS_API_TOKEN;
			if (!SS_API_TOKEN || url.searchParams.get('token') !== SS_API_TOKEN) return 返回SS接口JSON({ code: 401, success: false, msg: 'Unauthorized', data: [] }, 401);
			const rawNodes = await env.KV?.get('SOCKS5_NODES');
			if (!rawNodes) return 返回SS接口JSON({ code: 404, success: false, msg: 'SOCKS5_NODES not configured', data: [] }, 404);
			let socks5Nodes;
			try {
				socks5Nodes = JSON.parse(rawNodes);
			} catch (error) {
				return 返回SS接口JSON({ code: 500, success: false, msg: 'Invalid SOCKS5_NODES JSON', data: [] }, 500);
			}
			const 可用SOCKS5节点 = 校验并整理SOCKS5节点(socks5Nodes);
			if (可用SOCKS5节点.length === 0) return 返回SS接口JSON({ code: 404, success: false, msg: 'No available SOCKS5 nodes', data: [] }, 404);
			let count = Number.parseInt(url.searchParams.get('count') || '1', 10);
			if (!Number.isFinite(count) || count < 1) count = 1;
			if (count > 20) count = 20;
			const data = 随机打乱数组([...可用SOCKS5节点]).slice(0, count).map(node => ({ ip: node.ip, port: node.port, socks5: 生成SOCKS5链接(node) }));
			return 返回SS接口JSON({ code: 0, success: true, msg: '0', data });
		} else if (访问路径 === 'api/ss' && request.method === 'GET') {// SS节点接口
			const SS_API_TOKEN = env.SS_API_TOKEN;
			if (!SS_API_TOKEN || url.searchParams.get('token') !== SS_API_TOKEN) return 返回SS接口JSON({ code: 401, success: false, msg: 'Unauthorized', data: [] }, 401);
			const rawSSNodes = await env.KV?.get('SS_NODES');
			if (!rawSSNodes) return 返回SS接口JSON({ code: 404, success: false, msg: 'SS_NODES not configured', data: [] }, 404);
			let ssNodes;
			try {
				ssNodes = JSON.parse(rawSSNodes);
			} catch (error) {
				return 返回SS接口JSON({ code: 500, success: false, msg: 'Invalid SS_NODES JSON', data: [] }, 500);
			}
			const 可用SS节点 = 校验并整理SS节点(ssNodes);
			if (可用SS节点.length === 0) return 返回SS接口JSON({ code: 404, success: false, msg: 'No available SS nodes', data: [] }, 404);
			let count = Number.parseInt(url.searchParams.get('count') || '1', 10);
			if (!Number.isFinite(count) || count < 1) count = 1;
			if (count > 20) count = 20;
